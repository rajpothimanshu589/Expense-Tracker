...
const UpdateExpense = ({ expense }) => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.put(`http://localhost:8000/api/v1/expense/update/${expense._id}`,
      ...
      if (res.data.success) {
        const updatedExpense = expenses.map(exp => exp._id === expense._id ? res.data.expense : exp);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        ...
        }} size='icon' className="rounded-full border-green-600 text-green-600 hover:border-transparent" variant="outline"><Edit2 /></Button>
      </DialogTrigger>

      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Update Expense</DialogTitle>
          <DialogDescription>Update expense here. Click update when you're done.</DialogDescription>
        </DialogHeader>
